...
contract ERC20Airdroper is IUtilityContract, Ownable {
    ...
    function airdrop(address[] calldata receivers, uint256[] calldata amounts) external onlyOwner {
        ...
        require(token.allowance(treasury, address(this)) >= amount, "not enought approved tokens");

        for (uint256 i = 0; i < receivers.length; i++) {
            require(token.transferFrom(treasury, receivers[i], amounts[i]), "transfer failed");
        }
